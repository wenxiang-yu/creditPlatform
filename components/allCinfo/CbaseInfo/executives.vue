<template>
	...
	<view class="factorySHOW">
		...
		<view class="holderList">
			<view class="options" v-for="(item,index) in EmployeesList" :key="index">
			<view class="top">
			<view class="left">
				<view
				  style="
			...
			<view class="right">
				<view>{{item.name}}</view>
				<view class="inOffice" v-if="item.tag">
				  {{item.tag}}
				</view>
			</view>						
		</view>
	...
	<view class="NOdata" v-if="!EmployeesList.length">
...
<script>
	...
	export default {
		...
		data() {
			return {
				execuInfoList:[
					{title:"职务",content:"job"},
					{title:"公司数量",content:"companyNum"},
				]
			}
		},
		methods: {
