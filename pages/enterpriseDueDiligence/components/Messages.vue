...
<template>
  <view class="messages">
    <scroll-view
      ...
      ref="scroll"
    >
      <view v-for="(item, index) in messages" :key="index" class="bubble">
        <image
          style="width: 60rpx; height: 60rpx"
          src="/static/index/robot.png"
        ></image>
        <view v-if="getHTML(item).components === 'Kyc'" class="kycBtn">
          <view>
            <image
          ...
          <button type="primary" class="btn" @click="to('kyc', item)">
            立即查看
          </button>
        </view>
        <view v-else v-html="getHTML(item).st"></view>
      </view>
    </scroll-view>
  </view>
</template>
