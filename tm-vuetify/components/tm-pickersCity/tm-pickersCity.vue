<template>
  <view class="tm-pickersCity d-inline-block fulled">
    ...
    <tm-poup
	  ...
	  :over-close="false"
      :height="750"
      :bg-color="black_tmeme ? 'grey-darken-5' : bgColor"
    >
      <view class="tm-pickersCity-title pa-32 pb-16">
        <view class="text-size-g text-align-center" style="min-height: 48rpx">{{
          title
        }}</view>
        <view
          class="tm-pickersCity-close rounded flex-center"
          :class="black_tmeme ? 'grey-darken-3' : 'grey-lighten-3'"
        >
          <tm-icons
            @click="close"
            name="icon-times"
            size="24"
            :color="black_tmeme ? 'white' : 'grey'"
          ></tm-icons>
        </view>
      </view>
      <tm-pickersView
        v-if="showpop"
        @aniStart="aniisTrue = false"
        @aniEnd="aniisTrue = true"
      ...
      <view class="pa-32 btns_group">
		<view class="one" @click="reset" v-if="resetBtn">
			重置
		</view>
		<view class="two"  @click="confirm">
			{{ btnText }}
		</view>
      </view>
    </tm-poup>
  ...
  methods: {
    ...
    getSelectedValue() {
      ...
      if (this.level == "province") {
        p = [{name:d[0].data.text,adcode:d[0].data.id}];
      } else if (this.level == "city") {
        p = [{name:d[0].data.text,adcode:d[0].data.id},{name:d[1].data.text,adcode:d[1].data.id}];
      } else {
        p = [{name:d[0].data.text,adcode:d[0].data.id},{name:d[1].data.text,adcode:d[1].data.id},{name:d[2].data.text,adcode:d[2].data.id}];
      }
      //返回对象数组
