<template>
	<view class="tm-pickers d-inline-block fulled">
		...
		<tm-poup @change="toogle" ref="pop" v-model="showpop" :height="750" :bg-color="black_tmeme?'grey-darken-5':bgColor" :overClose="false">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 重置
			reset(){
				if(this.defaultrest === 1){
					this.$emit("resetIndustry")
				}else{
					this.$emit("restMore")
				}
